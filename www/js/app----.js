...
(function($, owner) {
	...
	owner.login = function(loginInfo, callback) {
		...
		/////
		mui.init();
		mui.post('http://td.sthcwh.com/td2017.php',{ name:loginInfo.account,psw:loginInfo.password,toke:text1},//请求，地址是服务器本机的ip地址//要传送的数据
	    function(data){ //服务器返回数据回来时应该做什么的函数
	   if(data=='1'){
	     localStorage.setItem('username',loginInfo.account);
	     localStorage.setItem('usertoke',text1);
